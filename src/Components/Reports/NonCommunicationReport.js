...
function NonCommunicationreport(){
    const [office,setOffice]=useState("3459274e-f20f-4df8-a960-b10c5c228d3e");
        const [Date, setDate] = useState();
        const [searchKey, setSearchKey] = useState();
        const [loading,setLoading]=useState();
        const[Noncommunicationdata,setNonCommunicationdata]=useState([]);
    ...
    const data = [{
      ...
      const ColumnDefs=[
        ...
        { headerName: "Duration", field: "Duration",flex: 2, minWidth: 200, maxWidth: 350,filter: true,  valueFormatter: (params) => params.value ? params.value : "--", },
      ...
      const baseUrl = "/api/server3/UHES-0.0.1/WS/ServerpaginationForNonCommunicationReportInReports?applyMaskingFlag=1&draw=2&length=10&nonCommunicationDate=20250131&officeId=3459274e-f20f-4df8-a960-b10c5c228d3e&start=0";
      const fetchAllNoncommunicationreport = async () => {
        setLoading(true);
        try {
          ...
          setNonCommunicationdata(responseData.data);
          setsubmitted(true)
      ...
      const exportExcel = async () => {
        await fetchAllNoncommunicationreport();
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Noncommunicationdata');
        const headers = ['SS NAME', 'METER INTERFACE', 'DURATION','GATEWAY','CONSUMER NUMBER','SIM NUMBER','CONSUMER NAME','METER CATEGORY', 'METER TYPE', 'CONTRACTED','METER LASTCOMMUNICATED', 'DTR NAME','METER NUMBER','FEEDER NAME','ZONE NAME']
        const headerRow = worksheet.addRow(headers);
        headerRow.eachCell((cell) => {
            cell.font = { bold: true };
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF00' } };
        });
        Noncommunicationdata.forEach((data) => {
            worksheet.addRow([
                data['SS Name'],
                data.meterInterface,
                data.Duration,
        ...
        saveAs(blob, 'NonCommunicationReport.xlsx');
    };
    
    const exportCSV = async () => {
        await fetchAllNoncommunicationreport();
        const worksheet = XLSX.utils.json_to_sheet(Noncommunicationdata);
        const csv = XLSX.utils.sheet_to_csv(worksheet);
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        saveAs(blob, 'NonCommunicationReport.csv');
    };
    ...
    const exportPDF = async () => {
        ...
        doc.autoTable({
            head: [['SS NAME', 'METER INTERFACE', 'DURATION','GATEWAY','CONSUMER NUMBER','SIM NUMBER','CONSUMER NAME','METER CATEGORY', 'METER TYPE', 'CONTRACTED','METER LASTCOMMUNICATED', 'DTR NAME','METER NUMBER','FEEDER NAME','ZONE NAME']
          ],
            body: Noncommunicationdata.map((data) => [
              data['SS Name'],
              data.meterInterface,
              data.Duration,
              data.GATEWAY_SIM_N,
        ...
        doc.save("NonCommunicationReport.pdf");
    };

    const handleSubmit = (event) => {
      event.preventDefault();
      fetchAllNoncommunicationreport();  
      setsubmitted(true);
    };
    ...
    const searchData = (e) => {
      ...
      if (searchValue === "") {
        setNonCommunicationdata(Noncommunicationdata);
      } else {
        const filteredData = Noncommunicationdata.filter((row) =>
          Object.values(row).some((val) =>
            String(val).toLowerCase().includes(searchValue.toLowerCase())
          )
        );
        setNonCommunicationdata(filteredData);
      }
    };
 ...
 export default NonCommunicationreport;