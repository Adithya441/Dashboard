...
function DeviceMaster(){
    ...
    const data = [{
      ...
      const baseUrl = "/api/server3/UHES-0.0.1/WS/ServerPaginationForDeviceMasterReport?Office=3459274e-f20f-4df8-a960-b10c5c228d3e&draw=2&length=73&start=0";
      const fetchAlldeviceMasters = async () => {
        setLoading(true);
        try {
          ...
          setdeviceMasters(responseData.data);
          allmasters(responseData.data);
          setsubmitted(true)
      ...
      const handleSubmit = (event) => {
        ...
        fetchAlldeviceMasters();  
        setsubmitted(true);
      };
      
      const ColumnDefs = [
        { headerName: "Sim No", field: "SIM_NO",flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Network ID", field: "NETWORK_ID",flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Longitude", field: "LONGITUDE", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "File Name", field: "FILE_NAME", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Mac Address", field: "MAC_ADDRESS", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Port No", field: "PORT_NO", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Network Type", field: "NETWORK_TYPE", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Field Device ID", field: "FIELD_DEVICE_ID", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Ip Address", field: "IP_ADDRESS", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Protocol Name", field: "PROTOCOL_NAME", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Latitude", field: "LATITUDE",flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Office Name", field: "OFFICE_NAME", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Field Device Type", field: "FIELD_DEVICE_TYPE", flex: 2, minWidth: 200, maxWidth: 350,filter: true },
        { headerName: "Nic-Details", field: "IMEI_NUMBER", flex: 2, minWidth: 200, maxWidth: 350,filter: true }

    ]

    const exportExcel = async () => {
        await fetchAlldeviceMasters();
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Device Masters');
        const headers = ['Sim No', 'Network ID', 'Longitude', 'File Name', 'Mac Address', 'Port No', 'NETWORK TYPE','Field Device ID','Ip Address','Protocol Name','Latitude','Office Name','Field Device Type','Nic-Details'];
        const headerRow = worksheet.addRow(headers);
        headerRow.eachCell((cell) => {
          cell.font = { bold: true };
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF00' } };
        });
        deviceMasters.forEach((data) => {
          worksheet.addRow([data.SIM_NO,
             data.NETWORK_ID, data.LONGITUDE, data.FILE_NAME, data.MAC_ADDRESS, data.PORT_NO, data.NETWORK_TYPE, data.FIELD_DEVICE_ID,data.IP_ADDRESS,data.PROTOCOL_NAME,data.LATITUDE,data.OFFICE_NAME,data.FIELD_DEVICE_TYPE,data.IMEI_NUMBER]);
        });
        ...
        saveAs(blob, 'DeviceMaster.xlsx');
      };
     

      const exportCSV = async () => {
        await fetchAlldeviceMasters();
        const worksheet = XLSX.utils.json_to_sheet(deviceMasters);
        const csv = XLSX.utils.sheet_to_csv(worksheet);
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        saveAs(blob, 'DeviceMasters.csv');
      };
      ...
      const exportPDF = async () => {
        ...
        doc.autoTable({
          head: [['Sim No', 'Network ID', 'Longitude', 'File Name', 'Mac Address', 'Port No', 'NETWORK TYPE','Field Device ID','Ip Address','Protocol Name','Latitude','Office Name','Field Device Type','Nic-Details']],
          body: deviceMasters.map((data) => [
            data.SIM_NO,
            data.NETWORK_ID, data.LONGITUDE, data.FILE_NAME, data.MAC_ADDRESS, data.PORT_NO, data.NETWORK_TYPE, data.FIELD_DEVICE_ID,data.IP_ADDRESS,data.PROTOCOL_NAME,data.LATITUDE,data.OFFICE_NAME,data.FIELD_DEVICE_TYPE,data.IMEI_NUMBER
          ]),
          startY: 20,
        });
        doc.save("DeviceMaster.pdf");
      };
      ...
      const searchData = (e) => {
        ...
        if (searchValue === "") {
          setdeviceMasters(deviceMasters);
        } else {
          const filteredData = deviceMasters.filter((row) =>
            Object.values(row).some((val) =>
              String(val).toLowerCase().includes(searchValue.toLowerCase())
            )
          );
          setdeviceMasters(filteredData);
        }
      };
        return (
          <Fragment>
        <Container fluid>
          ...
          <Col xs={12} md={4} className="ms-auto">
           ...
           {deviceMasters && submitted && office && (
            <div
              className="container-fluid ag-theme-quartz  mx-auto "
              style={{ height:500, width: "100%" }}
            >
              <AgGridReact
                rowData={deviceMasters}
